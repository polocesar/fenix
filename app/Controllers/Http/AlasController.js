...
class AlasController {
    ...
    async index ({ request, response, view, params }) {
        ...
        return {
            alas,
            total: await Leito.query().where('ala', q).getCount(),
            ...
            interditado: await Leito.query().whereIn('situacao', ['Interdição', 'Em Limpeza', 'Interditado por Infecção', 'Interditado Temporariamente']).where('ala', q).getCount(),
            altas_atrasadas: await Paciente.query()
                                            .innerJoin('paciente_leito', 'paciente_leito.cod_atendimento', 'pacientes.cod_atandimento')
    ...
    async indexx ({ request, response, view, params }) {
        let total, ocupados, livres, alas;
        let { q } = request.all();
