            ...
            <div class="card shadow mb-4">
                <form @submit.prevent="onSubmit()">
                    <div class="card-header py-3">
                        <h6 class="m-0 font-weight-bold text-default">Atualização de usuário</h6>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-6">
                        ...
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <input type="password" v-model="formulario.password" class="form-control" :class="{'is-invalid': errors.password}" id="password">
                            ...
                            <div class="col-lg-6">
                                <div class="form-group">
                                    ...
                                    <input type="password" v-model="formulario.password_confirmation" class="form-control" :class="{'is-invalid': errors.password_confirmation}" id="password_confirmation">
                    ...
                    <div class="card-footer">
                        <div class="col-12">
                            <router-link type="button" :to="{ name: 'usuario.index' }" tag="button" class="btn btn-danger font-weight-bold">
                                Cancelar
                            </router-link>
                            <button type="submit" class="btn float-right font-weight-bold btn-primary">Atualizar</button>
                        </div>
                    </div>
                </form>
            </div>
...
export default {
    ...
    async mounted () {
        ...
        try {
            const { data: { nome, email, grupo } } = await axios.get(`/api/user/${this.id}`, {
                headers: {
    ...
    methods: {
        async onSubmit() {
            ...
            Object.keys(this.formulario).forEach((key) => {
                if (this.formulario[key] != null && this.formulario[key].trim() != '') {
                    form[key] = this.formulario[key];
                }
            });
            try {
                await axios.put(`/api/user/${this.id}`, form, {
                    headers: {
                        'Authorization': 'Bearer ' + localStorage.getItem('token')
                    }
                });
